...
#ifndef CRILL_PROGRESSIVE_BACKOFF_WAIT_IMPL_CV_HYBRID_H
#define CRILL_PROGRESSIVE_BACKOFF_WAIT_IMPL_CV_HYBRID_H
...
namespace crill::impl
{
    ...
    void progressive_backoff_wait_intel_cv_hybrid(Predicate&& pred)
    {
        // phase 1
        for (int i = 0; i < N0; ++i)
        {
        ...
        while (true) {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(N4));
        }
    }
  #endif // CRILL_INTEL
    ...
    void progressive_backoff_wait_armv8_cv_hybrid(Predicate&& pred)
    {
        // phase 1
        for (int i = 0; i < N0; ++i)
        {
        ...
        while (true) {
            ...
            // waiting longer than we should, let's give other threads a chance to recover
            std::this_thread::sleep_for(std::chrono::milliseconds(N3));
        }
    }
