...
#ifndef CRILL_PROGRESSIVE_BACKOFF_WAIT_CV_H
#define CRILL_PROGRESSIVE_BACKOFF_WAIT_CV_H

#include <crill/platform.h>
#include <crill/impl/progressive_backoff_wait_impl_cv.h>
...
void progressive_backoff_wait_cv(Predicate&& pred)
{
  #if CRILL_INTEL
    impl::progressive_backoff_wait_intel_cv <5, 10, 50, 20, 3>(std::forward<Predicate>(pred));
    // approx:
    ...
    // - 3x350 µs (1.05 ms) 10k pauses (time estimated as above)
    // these are extrapolated from Timur's original benchmarks on a 2.9 GHz Intel i9
  #elif CRILL_ARM_64BIT
    impl::progressive_backoff_wait_armv8_cv<2, 10, 25, 3>(std::forward<Predicate>(pred));
    // approx:
    ...
    // - 3x333.25 µs (~ 1 ms), 250 pauses (time estimated as above)
